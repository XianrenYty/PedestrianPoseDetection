...
class StreamSpatialTemporalGraph(nn.Layer):
    ...
    def __init__(self, in_channels, graph_args, num_class=None,
        ...
        A = paddle.to_tensor(graph.A, dtype='float32')
        ...
        if edge_importance_weighting:
            self.edge_importance = nn.ParameterList([
                paddle.create_parameter(A.shape, dtype='float32')
            for i in range(len(self.st_gcn_networks))])
            new_state_dict = OrderedDict()
            for k, v in self.edge_importance.state_dict().items():
                new_state_dict[k] = paddle.to_tensor(paddle.ones(v.shape), stop_gradient=False)
            self.edge_importance.set_state_dict(new_state_dict)
        else:
...
class TwoStreamSpatialTemporalGraph(nn.Layer):
