...
if __name__ == '__main__':
    ...
    par.add_argument('--device', type=str, default='cuda', help='depracted: Device to run model on cpu or cuda.')
    args = par.parse_args()
    ...
    while cam.grabbed():
        ...
        for track in tracker.tracks:
            det = paddle.to_tensor([track.to_tlbr().tolist() + [0.5, 1.0, 0.0]], dtype='float32')
            detected = paddle.concat([detected, det], axis=0) if detected is not None else det
